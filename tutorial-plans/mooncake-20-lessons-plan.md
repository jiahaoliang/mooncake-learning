...
- 所有课程必须遵守 `tutorials/00-writing-guide.md`：术语首次出现前必须解释，不能只给 glossary 链接。
...
- 每新增一课，同步更新 `tutorials/README.md` 推荐阅读顺序。
- 新增术语先补 `tutorials/glossary.md`，正文首次出现时就地解释。
...
- 本文件只记录规划，不代表正式阅读顺序已经更新；正式新增课程时再同步 `tutorials/README.md`。
