...
后续课程会沿着 Mooncake 主线展开：先理解 Mooncake 的整体架构和数据通路，再分别进入 Transfer Engine、P2P KV 传输、Mooncake Store、Prefix Cache 和源码专题。`MooncakeConnector` 和 `MooncakeStoreConnector` 是具体代码实现名，本文前面已经解释过它们分别偏向点对点传输和共享缓存池。
